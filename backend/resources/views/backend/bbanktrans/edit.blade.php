...
@include('backend.layouts.notification')
    ...
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('bbanktrans.update',$bbanktran->id)}}">
                @csrf
                @method('patch')
                <div class="intro-y box p-5">
                    <div class="mt-6">
                        ...
                        <select name="bank_id" class="form-select mt-2 sm:mr-2"   >
                            @foreach ($banklist as $bank)
                                    <option value ="{{$bank->id}}" {{$bank->id==$bbanktran->bank_id?"selected":""}} >{{$bank->title}}</option>
                            @endforeach    
                        </select>
                    ...
                    <div>
                        ...
                        <input id="amount" name="amount" type="text" class="form-control" value="{{$bbanktran->amount}}" >
